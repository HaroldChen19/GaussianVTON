...
class InstructPix2PixGuidance(BaseObject):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # print("cond_rgb:", cond_rgb.shape)
        # image_data = (cond_rgb[0].cpu().numpy() * 255).astype(np.uint8)
        # save_path = f'/root/autodl-tmp/GaussianVTON/test/01_0.jpg'
        # image_data = cv2.cvtColor(image_data, cv2.COLOR_RGB2BGR)  # PIL使用RGB格式，而OpenCV使用BGR格式
        ...
        if self.cfg.use_sds:
            ...
            # editimage = edit_images.permute(0, 2, 3, 1)
            # image_data = (editimage[0].cpu().numpy() * 255).astype(np.uint8)
            # save_path = f'/root/autodl-tmp/GaussianVTON/test/02_0.jpg'
            # image_data = cv2.cvtColor(image_data, cv2.COLOR_RGB2BGR)  # PIL使用RGB格式，而OpenCV使用BGR格式
            # cv2.imwrite(save_path, image_data)
            # print("edit_images:", editimage.shape)
