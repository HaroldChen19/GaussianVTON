...
class WebUI:
    def __init__(self, cfg) -> None:
        ...
        if self.colmap_dir is not None:
            ...
            images_dir = os.path.join(self.colmap_dir, "images")
            ...
            if image_files:
                ...
                with Image.open(first_image_path) as img:
                    ...
                    print(w)
                    print(h)
                    scene = CamScene(self.colmap_dir, h, w)
        ...
        with self.server.add_gui_folder("Edit Setting"):
            ...
            self.guidance_type = self.server.add_gui_dropdown(
                "Guidance Type", ("InstructPix2Pix", "ControlNet-Pix2Pix", "Ladi-Vton")
            )
            ...
            self.edit_image = self.server.add_gui_text(
                "Image",
                initial_value="",
                visible=True,
            ) #value should be the path of image prompt
    ...
    def edit(self, edit_cameras, train_frames, train_frustums):
        ...
        elif self.guidance_type.value == "Ladi-Vton":
            if not self.ladi:
                from threestudio.models.guidance.ladi_guidance import (
                    LadiVtonGuidance,
                )
                self.ladi = LadiVtonGuidance()
            cur_2D_guidance = self.ladi
            print("using Ladi-Vton!")
        ...
        self.guidance = EditGuidance(
            ...
            text_prompt=self.edit_text.value,
            image_prompt=self.edit_image.value,
        ...
        view_index_stack = list(range(len(edit_cameras)))
        for step in tqdm(range(self.edit_train_steps.value)):
            # if self.edit_image.value is not None:
            #     rendering_results = []
            #     for view_index in view_index_stack:
            #         rendering = self.render(edit_cameras[view_index], train=True)["comp_rgb"]
            #         rendering_results.append(rendering)
            #     rendering_results = torch.cat(rendering_results, dim=0)
            #     print("rendering_results:", rendering_results.shape)
            #     loss = self.guidance(rendering_results, view_index_stack, step)
            #     loss.backward()
    ...
    def add_theme(self):
        ...
        image = TitlebarImage(
            image_url_light="/root/autodl-tmp/GaussianVTON/logo.jpg",
            image_alt="GaussianVTON Logo",
            href="https://github.com/",
        )
